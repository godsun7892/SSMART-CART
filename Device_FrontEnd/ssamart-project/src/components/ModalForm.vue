    ...
    <div class="product-location-modal" @click="closeModal" >
        <div class="modal-content" @click.stop="">
            <div class="modal-buttons">
                <button @click="switchModal(0)" :class="[flagSwitch.activeButtonIndex === 0 ? 'whenButtonClicked' : 'selectBtn']">상품 검색</button>
                <button @click="switchModal(2)" :class="[flagSwitch.activeButtonIndex === 2 ? 'whenButtonClicked' : 'selectBtn']">매장 지도</button>
                <button @click="switchModal(3)" :class="[flagSwitch.activeButtonIndex === 3 ? 'whenButtonClicked' : 'selectBtn']">오늘의 핫딜</button>
            </div>
            <div class="btnRange">
            ...
            <searchScreen v-if="screenIndex===0"/>
            <!-- <searchResult v-if="screenIndex===1"/> -->
            <ProductLocation v-if="screenIndex===2"/>
            <HotDealScreen v-if="screenIndex===3"/>
        </div>
...
// import searchResult from "@/components/searchResultScreen.vue";
import ProductLocation from "@/components/ProductLocationModal.vue";
import HotDealScreen from "@/components/HotDeal2.vue";

const flagSwitch = useFlagStore();
const {screenIndex} = storeToRefs(flagSwitch);
...
.modal-buttons button:hover {
    background-color: rgb(241, 239, 239); /* 호버 배경색 */
...
.whenButtonClicked {
    border : none;
    background-color: white;
    border-bottom: 5px solid #80E1FF;
    font-weight: 40px;
    color: #80E1FF;
}
.mapBtn {
