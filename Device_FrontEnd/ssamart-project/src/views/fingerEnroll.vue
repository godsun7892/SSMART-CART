        ...
        <div class="updown">
            <div class="left">
                <div class="mainSection">
                    <div class="phoneInfo">
                        <div class="txt"><b>지문</b>을 등록해 <br>주세요.</div>
                    </div>
                    <div class="notUsermessage">
                        <div class="sideline"></div>
                        <div class="notUserText">나중에 등록할래요.</div>
                        <div class="sideline"></div>
                    </div>
                    
                    <button class="nextBtn notUser" @click="exitFp">나중에 등록하기</button>
                </div>
            </div>
            <div class="right">
                <div class="fingetText">화면 아래 지문 인식기에<br><b>오른손 검지손가락</b>을 대주세요.<br><b>( {{ cur_idx }} / 3 )</b></div>
...
const {loadingFlag} = storeToRefs(flagStore);
...
const exitFp = () =>{
    ...
    loginStore.loginMessageModal(3)
}


const registerFunc = (phoneNum,data) => {
		//console.log(data)
		axios
			...
			.then((response) => {
                if(response.status===200){
                    //fncMqttDoSend("CANCEL");
                    loginStore.loginMessageModal(2);
                }
...
$(document).ready(function(){
        ...
        function onMessageArrived(message) 
        {
            ...
            if(msgLen <= 15 ){
                ...
                // 이미지 카운팅 추가 필요
                if(!loadingFlag.value) loadingFlag.value = true;
            }
            else if (msgLen > 15){
                ...
                if(cur_idx.value < max_idx){
                    console.log('cur_idx: ' + cur_idx.value)
                    databuf[cur_idx.value] = biArray
                    cur_idx.value += 1
                    cnt = 0;
                ...
                if(cur_idx.value >= max_idx){
                    ...
                    loadingFlag.value = false;
                    registerFunc(userPhoneNumber,databuf.slice(0,4))
                    cur_idx.value = 0
                    databuf = []
                }
                else{
