...
struct Extended_Packet {
  ...
  */
  ...
  Extended_Packet(uint8_t type, uint16_t length, uint8_t *data) {
    this->start_code = FINGERPRINT_STARTCODE;
    this->type = type;
    this->length = length;
    address[0] = 0xFF;
    ...
    if (length < 130)
      ...
      memcpy(this->data, data, 150);
  }
  uint16_t start_code; ///< "Wakeup" code for packet detection
  uint8_t address[4];  ///< 32-bit Fingerprint sensor address
  uint8_t type;        ///< Type of packet
  uint16_t length;     ///< Length of packet
  uint8_t data[130];    ///< The raw buffer for packet payload
};
...
public:
  Adafruit_Fingerprint(char* device, uint32_t password = 0x0);
  ...
  uint8_t LEDcontrol(uint8_t control, uint8_t speed, uint8_t coloridx,
                     uint8_t count = 0);
  uint8_t upImageCmd(void);
  uint8_t upImage(uint8_t img_Buffer[288][128]);
  ...
  uint8_t getExtendedPacket(Extended_Packet *p, uint16_t timeout = DEFAULTTIMEOUT);
  bool getImageData(uint8_t bytesReceived[534]);
